...
if ($rota) {
    ...
    if ($veiculoRota && $veiculoRota['disponibilidade'] == 0) { // Veículo em uso
        ...
        if (empty($rota['km_final']) || empty($rota['data_final']) || empty($rota['hora_final'])) {
            ...
            echo "
            <script>
                // Enviar o rota_id automaticamente para buscarDestinos.php sem redirecionar
                (function sendRotaId() {
...
<body id="pagina-inicial">
    ...
    <div class="veiculos-lista">
        ...
        <?php if ($exibirModalRota && $rotaAtiva): ?>
            ...
            <div id="modalRotaAtiva" class="modal2" style="display: none;">
                <div class="modal2-content">
                    <h2>Rota Ativa</h2>
                    <p><strong>Finalize a rota para sair da conta!</strong></p>
                    <button onclick="rotaAtivaClose()">Ok</button>
                </div>
            </div>
        ...
        <?php
            ...
            while($veiculo = $result->fetch_assoc()):
                ...
                if ($veiculo['disponibilidade'] == 0 && $veiculo['rota_funcionario_id'] == $usuarioID) {
                    $classeDisponibilidade = "rota-veiculos-selecionar";
                    $imagemVeiculo = "../img/icone_veiculo_rota.png";
                    $textoDisponibilidade = "Em Rota";
                    ...
                    $rotaEmAndamento = true; // Define que o usuário já está em rota
    ...
    <form action="veiculoSelecionado.php" method="POST">
        <button type="submit" <?= $disabled; ?>>
            <div class="veiculos-container">
                <div class="veiculos-container-before"></div>
                <div class="<?= !empty($classeDisponibilidade) ? $classeDisponibilidade : 'indisp-veiculos-selecionar'; ?>">
                    <div class="<?= !empty($bloco) ? $bloco : 'indisp'; ?>-veiculos-bloco">
                        <div class="<?= !empty($dados) ? $dados : 'indisp'; ?>-veiculos-dados">
                            <img src="<?= !empty($imagemVeiculo) ? $imagemVeiculo : '../img/icone_veiculo_cinza.png'; ?>" alt="">
                        </div>
                        <div class="<?= !empty($dados) ? $dados : 'indisp'; ?>-veiculos-dados <?= !empty($corBorda) ? $corBorda : 'indisp'; ?>-borda-direita">
                            <h3 style="<?= !empty($corTexto) ? $corTexto : ''; ?>">Placa</h3>
                            <p style="<?= !empty($corTexto) ? $corTexto : ''; ?>"><?= htmlspecialchars($veiculo['placa']); ?></p>
                        </div>
                        <div class="<?= !empty($dados) ? $dados : 'indisp'; ?>-veiculos-dados <?= !empty($corBorda) ? $corBorda : 'indisp'; ?>-borda-direita">
                            <h3 style="<?= !empty($corTexto) ? $corTexto : ''; ?>">Tombamento</h3>
                            <p style="<?= !empty($corTexto) ? $corTexto : ''; ?>"><?= htmlspecialchars($veiculo['tombamento']); ?></p>
                        </div>
                        <div class="<?= !empty($dados) ? $dados : 'indisp'; ?>-veiculos-dados">
                            <img src="<?= !empty($iconeDisponibilidade) ? $iconeDisponibilidade : '../img/icone_cadeado_cinza.png'; ?>" alt="">
                            <div class="<?= !empty($statusVeiculo) ? $statusVeiculo : 'indisp'; ?>-informativo-label">
                                <?= !empty($textoDisponibilidade) ? $textoDisponibilidade : 'Indisponível'; ?>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <div class="veiculos-titulo">
        <div class="veiculos-titulo-before"></div>
        <h2>Você está em rota</h2>
    </div>
    <div class="veiculos-lista">
        <form action="finalizarRota.php" method="POST">
            <button type="submit">
                <div class="veiculos-container">
                    <div class="veiculos-container-before"></div>
                    <div class="rota-veiculos-selecionar">
                        <div class="rota-veiculos-bloco">
                            <div class="rota-veiculos-dados">
                                <img src="../img/icone_veiculo_selecionar.png" alt="">
                            </div>
                            <div class="rota-veiculos-dados <?= $corBorda; ?>-borda-direita">
                                <h3>Placa</h3>
                                <p><?= htmlspecialchars($veiculo['placa']); ?></p>
                            </div>
                            <div class="rota-veiculos-dados <?= $corBorda; ?>-borda-direita">
                                <h3>Tombamento</h3>
                                <p><?= htmlspecialchars($veiculo['tombamento']); ?></p>
                            </div>
                            <div class="rota-veiculos-dados">
                                <img src="../img/icone_seta_preta.png" alt="">
                                <div class="rota-informativo-label">Em rota</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <input type="hidden" name="tombamento" value='<?= htmlspecialchars($veiculo['tombamento']); ?>' />
                
            </button>
        </form>
        <p class="mensagem-indicativa">Selecione o veículo para finalizar a rota.</p>
    </div>
    <div class="container">
        <h1>Insira seu próximo destino</h1>
        <div id="destinos-list"></div>
        <div class="input-wrapper">
            <input type="text" class="input-invisivel" id="destino" placeholder="Digite um destino..." readonly onclick="abrirModal()">
        </div>
        <button class="button" onclick="abrirModal()">
            <div class="adicionar-itinerario-button">
                <p>Adicionar Itinerário</p>
                <img src="../img/icone_X_cinza.png" alt="">
            </div>
        </button>

        <hr style="margin: 30px 0; border: 1px solid #6C757D;">

        <h1>Abasteceu o veículo?</h1>
        <div id="lista-abastecimentos"></div>
        <div class="input-wrapper">
            <input type="text" class="input-invisivel" id="abastecimento" readonly onclick="abrirModalAbastecimento()">
        </div>
        <button class="button" onclick="abrirModalAbastecimento()">
            <div class="adicionar-abastecimento-button">
                <p>Adicionar Abastecimento</p>
                <img src="../img/icone_X_cinza.png" alt="">
            </div>
        </button>
    </div>
    ...
    <div id="modal" style="display:none;">
        <div class="modal-content">
            <h2>Adicionar Itinerário</h2>
            <textarea id="endereco" name="endereco" placeholder="Digite o endereço aqui..."></textarea>
            <button id="adicionar-destino-btn" class="modal-button" onclick="adicionarDestino()">Adicionar</button>
            <button class="modal-button" onclick="fecharModal()">Cancelar</button>
        </div>
    </div>

    <div id="modal-abastecimento" style="display:none;">
        <div class="modal-content">
            <h2>Adicionar Abastecimento</h2>
            <!-- Formulário para os dados do abastecimento -->
            <form id="form-abastecimento" enctype="multipart/form-data">
                <input type="number" id="litros" name="litros" maxlength="3" min="1" max="999" step="0.1" placeholder="Litros" required>
                <input type="number" id="km" name="km" maxlength="6" min="1" max="999999" step="1" placeholder="KM atual" required>

                <div class="custom-file-input">
                    <label for="comprovante" id="comprovante-label">Selecionar Comprovante</label>
                    <input type="file" id="comprovante" name="comprovante" accept=".png, .jpg, .jpeg" style="display: none;" onchange="handleFileSelect()" required>
                </div>

                <button type="button" class="modal-button" onclick="adicionarAbastecimento()">Adicionar</button>
                <button type="button" class="modal-button" onclick="fecharModalAbastecimento()">Cancelar</button>
            </form>
...
<div class="veiculos-lista" style="margin-bottom: 70px;">
    <div id="historicoVeiculos">
        ...
        if ($stmt) {
            ...
            if ($result->num_rows > 0) {
                ...
                <div class="veiculos-container-historico">  
                        ...
                        <div class="historico-veiculos-selecionar">
                            <div class="historico-veiculos-bloco">
                                ...
                                <div class="historico-veiculos-dados historico-borda-direita">
                                    <h3>Placa</h3>
                                    <p><?= htmlspecialchars($rota['placa']); ?></p>
                                </div>
                                <div class="historico-veiculos-dados historico-borda-direita">
                                    <h3>Data</h3>
                                    <?php
                                    $data_inicial = $rota['data_inicial'];
                                    $data_formatada = DateTime::createFromFormat('Y-m-d', $data_inicial)->format('d/m/Y');
                                    ?>
                                    <p><?= htmlspecialchars($data_formatada); ?></p>
                                </div>
                                <div class="historico-veiculos-dados">
    ...
    ?>
    <!-- Botão Ver Mais -->
    <div style="display: flex; justify-content: center; align-items: center;" class="ver-mais-container">

        <!-- Botão de Ver Mais -->
        <div id="verMais" class="historico-ver-mais"><p>Ver Mais</p></div>

        <!-- Novo contêiner para a mensagem -->
        <div id="verMaisMessage"></div>
